...
import uuid
from app.api.schemas import JobRequest, JobStatusResponse, PresignRequest, PresignResponse
from app.services.cache import set_job_status, get_job_status
from app.services.aws_client import generate_presigned_url
...
async def get_upload_url(payload: PresignRequest):
    """
    Step 1 of the workflow: the client asks for a secure, direct-to-S3 upload URL.
    The returned `object_name` is what should then be passed as `filename` when
    submitting a processing job via POST /jobs.
    """
    object_name = generate_unique_filename(payload.filename)
    upload_url = generate_presigned_url(object_name)

    if not upload_url:
        raise HTTPException(
            status_code=500,
            detail="Could not generate a presigned upload URL. Check AWS credentials/bucket configuration."
        )
